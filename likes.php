...
if (isset($_POST['quote_liked'])) {
	$quote_id = $_POST['quote_id'];
	$user_id = $_POST['user_id'];
	$result = mysqli_query($connection, "SELECT likes FROM quotes WHERE quote_id = $quote_id");
	$row = mysqli_fetch_array($result);
	$n = $row['likes'];

	// Insert user_id and quote_id into quote_likes table
	$query = "INSERT INTO quote_likes (user_id, quote_id) VALUES (?, ?)";
	$stmt = mysqli_prepare($connection, $query);
	mysqli_stmt_bind_param($stmt, 'ii', $user_id, $quote_id);
	mysqli_stmt_execute($stmt);
	mysqli_stmt_close($stmt);

	// Update likes in quotes table
	$query1 = "UPDATE quotes SET likes=$n+1 WHERE quote_id = ?";
	$stmt1 = mysqli_prepare($connection, $query1);
	mysqli_stmt_bind_param($stmt1, 'i', $post_id);
	mysqli_stmt_execute($stmt1);
	mysqli_stmt_close($stmt1);
...
if (isset($_POST['quote_unliked'])) {
	...
	$query = "DELETE FROM quote_likes WHERE quote_id = ? AND user_id = ? ";
	$stmt = mysqli_prepare($connection, $query);
	mysqli_stmt_bind_param($stmt, 'ii', $post_id, $user_id);
	mysqli_stmt_execute($stmt);
	mysqli_stmt_close($stmt);

	$query = "UPDATE quotes SET likes=$n-1 WHERE quote_id = ?";
	$stmt1 = mysqli_prepare($connection, $query);
	mysqli_stmt_bind_param($stmt1, 'i', $post_id);
	mysqli_stmt_execute($stmt1);
	mysqli_stmt_close($stmt1);
