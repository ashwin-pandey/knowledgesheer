...
if (isset($_GET['q_cat_id'])) {
	$post_category_id = $_GET['q_cat_id'];
...
<div class="row pt-4">
	...
	<div class="col-lg-6 col-md-6 col-12">
		<?php
		$post_query_count = "SELECT * FROM quotes WHERE quote_category = ".$post_category_id;
		$find_count = query($post_query_count);
		$count = mysqli_num_rows($find_count);
		if($count < 1) {
			echo "<h5 class='text-center'>No thoughts available</h5>";
		} else {
			$query = 'SELECT * FROM quotes WHERE quote_category = ' .$post_category_id .' ORDER BY quote_id DESC ';
